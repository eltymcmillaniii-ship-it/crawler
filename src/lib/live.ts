...
export async function loadCharacters(gameId: string): Promise<Character[]> {
  ...
  const [skillsRes, itemsRes, achievementsRes, boxesRes] = await Promise.all([
    ...
    sb.from('character_items').select('id,character_id,quantity,equipped_slot,tradeable,item:items(id,name,rarity,item_type,slot,effect,quirk,core_value,strength_bonus,dexterity_bonus,intelligence_bonus,constitution_bonus,charisma_bonus,ai_generated)').in('character_id', ids),
