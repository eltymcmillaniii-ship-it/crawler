alter table public.items add column if not exists strength_bonus integer not null default 0;
alter table public.items add column if not exists dexterity_bonus integer not null default 0;
alter table public.items add column if not exists intelligence_bonus integer not null default 0;
alter table public.items add column if not exists charisma_bonus integer not null default 0;

alter table public.items drop constraint if exists items_strength_bonus_check;
alter table public.items drop constraint if exists items_dexterity_bonus_check;
alter table public.items drop constraint if exists items_intelligence_bonus_check;
alter table public.items drop constraint if exists items_charisma_bonus_check;
alter table public.items add constraint items_strength_bonus_check check (strength_bonus between 0 and 999);
alter table public.items add constraint items_dexterity_bonus_check check (dexterity_bonus between 0 and 999);
alter table public.items add constraint items_intelligence_bonus_check check (intelligence_bonus between 0 and 999);
alter table public.items add constraint items_charisma_bonus_check check (charisma_bonus between 0 and 999);

drop function if exists public.gm_grant_item(uuid,text,public.item_rarity,text,text,text,text,integer,integer,integer);
...
begin
  ...
  values(
    ...
    p_strength_bonus,p_dexterity_bonus,p_intelligence_bonus,p_constitution_bonus,p_charisma_bonus,
    false
  )
...
revoke all on function public.gm_grant_item(uuid,text,public.item_rarity,text,text,text,text,integer,integer,integer,integer,integer,integer,integer) from public, anon;
grant execute on function public.gm_grant_item(uuid,text,public.item_rarity,text,text,text,text,integer,integer,integer,integer,integer,integer,integer) to authenticated;

create or replace function public.gm_adjust_item_stat_bonus(p_character_item_id uuid,p_stat text,p_delta integer)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select c.game_id,ci.item_id,ci.character_id into v_game_id,v_item_id,v_character_id
  ...
  if p_stat='Strength' then
    ...
    if exists(select 1 from public.character_items where id=p_character_item_id and equipped_slot is not null) then
...
begin
  ...
  if v_slot is not null and v_slot not in ('Head','Body','Hands','Feet','Weapon 1','Weapon 2','Accessory 1','Accessory 2') then v_slot := null; end if;
  ...
  insert into public.character_items(character_id,item_id,quantity) values(c.id,v_item,1) returning id into v_character_item;
...
begin
  ...
  values(p_game_id,auth.uid(),p_event_text,p_verdict,true) returning id into v_event_id;
  ...
  for v_recipient_text in select value from jsonb_array_elements_text(coalesce(p_verdict->'recipients','[]'::jsonb)) loop
    ...
    if v_kind='loot_box' and v_rarity_text in ('B','S','G') then
      insert into public.loot_boxes(character_id,name,rarity) values(v_recipient,coalesce(nullif(v_reward_name,''),'Dungeon Loot Box'),v_rarity_text::public.item_rarity);
      ...
      insert into public.skills(character_id,name,rank) values(v_recipient,coalesce(nullif(v_reward_name,''),'Unidentified Skill'),1)
      on conflict(character_id,name) do update set rank=public.skills.rank+1;
    elsif v_kind='perk' then
      update public.characters set perks=case when perks @> array[v_reward_name]::text[] then perks else array_append(perks,v_reward_name) end where id=v_recipient;
    elsif v_kind='party_reward' and v_rarity_text in ('B','S','G') then
      insert into public.loot_boxes(character_id,name,rarity) values(v_recipient,coalesce(nullif(v_reward_name,''),'Party Reward'),v_rarity_text::public.item_rarity);
    end if;
  end loop;
  return v_event_id;
end;
...
begin
  ...
  if v_item_type not in ('Weapon','Armor','Accessory','Consumable','Utility','Quest','AI Generated') then v_item_type:='AI Generated'; end if;
  if v_item_slot is not null and v_item_slot not in ('Head','Body','Hands','Feet','Weapon 1','Weapon 2','Accessory 1','Accessory 2') then v_item_slot:=null; end if;
