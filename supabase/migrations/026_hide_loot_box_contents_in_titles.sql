-- Keep loot-box titles secret so the player does not see the guaranteed contents before opening.
  ...
  for v_recipient_text in select value from jsonb_array_elements_text(p_command->'recipients') loop
    ...
    if v_kind in ('item','loot_box') then
      ...
      if v_distribution='individual' then
        ...
        limit 1;
        if v_selected_item is null then raise exception 'Missing individual item for recipient %',v_recipient_text; end if;
      end if;
